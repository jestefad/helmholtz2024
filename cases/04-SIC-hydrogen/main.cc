...
constexpr static std::size_t nspecies=13;
constexpr static std::size_t nreactions=33;
constexpr static std::size_t maxVibLevel=6;
...
int main(int argc, char** argv)
{
    ...
    env.exec([&](spade::parallel::pool_t& pool)
    {
        ...
        const auto ini = [=] _sp_hybrid (const point_type& x, const spade::grid::cell_idx_t& ii)
        {
			...
			coor_t Rsurf   = 0.0075;
			coor_t Rlocsp  = sqrt((x[0]-0.0075)*(x[0]-0.0075) + x[1]*x[1] + x[2]*x[2]);
			...
			if (x[0]<Rsurf)
			{
				...
				{
					// Velocity ramp
					output.u()   = Uinf * cos(theta) * spade::utils::min(1, distsp / rampMax);
					output.v()   = Uinf * sin(theta) * spade::utils::min(1, distsp / rampMax);
					output.w()   = 0.0;
				}
				else
				{
			...
			else
			{
				...
				{
					// Velocity ramp
					output.u()   = Uinf * cos(theta) * spade::utils::min(1, distcn / rampMax);
					output.v()   = Uinf * sin(theta) * spade::utils::min(1, distcn / rampMax);
					output.w()   = 0.0;
				}
				else
				{
