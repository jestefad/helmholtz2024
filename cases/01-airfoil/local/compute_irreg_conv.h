...
namespace detail{
	...
	// _sp_hybrid inline auto swapdiss(const spade::convective::hybrid_df_scheme_t<cent_t, diss_t, ducr_t>& base, const real_t& new_diss_val)
	// {
	// 	spade::state_sensor::const_sensor_t new_sensor(new_diss_val);
	// 	return spade::convective::hybrid_df_scheme_t(base.scheme0, base.scheme1, new_sensor);
	// }
}
...
namespace local
{
  template <
    ...
    typename vector_t,
    typename tscheme_t>
    inline void rhs_irreg_conv(
                               const array_t& prim,
                               rhs_t& rhs,
                               const gas_t& gas,
                               const tscheme_t& tscheme,
                               const ghost_t& ghost,
