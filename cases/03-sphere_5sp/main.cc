...
int main(int argc, char** argv)
{
    ...
    env.exec([&](spade::parallel::pool_t& pool)
    {
        ...
        if (pool.isroot()) print("Compute interp 2");
        auto interp2  = [&](){
            try
            {
            ...
            {
                spade::io::output_vtk("debug/failed2." + std::to_string(pool.rank()) + ".vtk", e.data);
                throw e;
            }
        }();
        if (pool.isroot()) print("Done");
        ...
        const auto ini = [=] _sp_hybrid (const point_type& x, const spade::grid::cell_idx_t& ii)
        {
			...
			// Initial condition
			for (int s = 0; s<output.ns-1; ++s) output.Ys(s) = Yinf[s];
		...
		std::string nstr = spade::utils::zfill(0, 8);
		...
		auto calc_rhs = [&](auto& rhs_in, const auto& prim_in, const auto& t)
     	{
            ...
            t3.start();
            //local::zero_ghost_rhs(rhs_in, ghosts); // CHECK THIS !!!!
            t3.stop();
            
            spade::timing::tmr_t t4;
            t4.start();
            //local::zero_rhs_inside(prim_in, grid, rhs_in, air5, inout_gpu); // CHECK THIS !!!!
            t4.stop();
        ...
        {
			prim_t qguard;
			for (int s = 0; s<qguard.ns-1; ++s) qguard.Ys(s) = Yinf[s];
			real_t theta = spade::consts::pi*aoa/180.0;
		...
		auto boundary_cond = [&](parray_t& q, const real_t& t)
     	{
            ...
            spade::sampling::sample_array(sampldata2, q, interp2);
            t4.stop();

			// Ghost filling
			spade::timing::tmr_t t5;
            t5.start();
            //local::fill_ghost_vals(q, ghosts, ips2, sampldata, sampldata2); // CHECK THIS !!!!
            t5.stop();
